...
int maxDepth(struct node * T) {//tìm độ xâu của 
	if (T==NULL)
		return 0;
	else {
...
unsigned int getLeafCount(Tree T) {//đếm số node lá trên cây
	if(T == NULL)
		return 0;
...
void printPathsRecur(Tree T, int path[], int pathLen)  {//tìm đường đi từ gốc đến lá.
	if (T==NULL) return;
